...
termintorX to syntezator dźwięku pracujący w czasie rzeczywistym,
pozwalający skreczować na cyfrowo spróbkowanych danych dźwiękowych
(*.wav, *.au, *.ogg, *.mp3, etc.) tak jak to robią DJ-e hiphopowi na
płytach winylowych. Jego możliwości to wiele talerzy, efekty w czasie
rzeczywistym (wbudowane, jak również wtyczki efekty LADSPA), sequencer
oraz interfejs MIDI - wszystko dostępne poprzez łatwe w użyciu GUI
napisane w GTK+.
